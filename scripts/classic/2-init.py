...
sample_ids = np.unique(lims["library"])
for iden in sample_ids:
    ...
    for new_dir in dirs_to_create:
        if not os.path.exists(new_dir):
            os.mkdir(new_dir) 
    bool_mask = (lims["library"] == iden) & (lims["libraryPassFail"] == "pass") & (lims["LanePassFail"] == "pass")
    print(bool_mask)
    ...
    count_id = np.sum(lims_sub["library"] == iden)
    if count_id > 1:
        ...
        subprocess.run(fastq_path_list_r1.insert(0, "cat"), stdout = open("{}/{}_S1_L001_R1_001.fastq.gz".format(fastq_dir, iden), "w"))
        subprocess.run(fastq_path_list_r2.insert(0, "cat"), stdout = open("{}/{}_S1_L001_R2_001.fastq.gz".format(fastq_dir, iden), "w"))
    else:
...
/scratch/devel/rmassoni/cellranger-3.0.2/cellranger count --fastqs {} --id {} --chemistry SC3Pv3 --expect-cells 5000 --localcores 8 --localmem 64 --sample {} --transcriptome {};
